...
{
    public class MindrayBS240E_IP
    {
        private static readonly ILog log4net = LogManager.GetLogger(typeof(MindrayBS240E_IP).Name);
        ...
        public static void ServerReceive(System.Object obj)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (isWriteFile) Folder_File_Tool.CreateWriteFileByDate(typeof(MindrayBS240E_IP).Name, true, dataRemovedByteNull);
                    byteStringData = Encoding.ASCII.GetBytes(stringData);

                    //if (byteStringData != null && byteStringData.Contains(ProtocolASCII.FS) && byteStringData.Contains(ProtocolASCII.CR))
                    //{
        ...
        public static void HostSend_ACK(string stringData)
        {
            ...
            //string stringACK = (char)ProtocolASCII.FS + @"SA||AA|250|Message accepted|||0|ACK^R01|250|P|2.3.1||||0||ASCII|||";
            byte[] byteData = Encoding.ASCII.GetBytes(stringACK);
            if (isWriteFile) Folder_File_Tool.Create_Write_File(typeof(MindrayBS240E_IP).Name, true, stringACK);
