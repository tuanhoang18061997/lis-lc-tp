...
{
    public class SDBIOSENSOR
    {
        private static readonly ILog log4net = LogManager.GetLogger(typeof(SDBIOSENSOR).Name);
        ...
        public static void ServerReceive(System.Object obj)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if(isWriteFile) Folder_File_Tool.Create_Write_File(typeof(SDBIOSENSOR).Name, true, dataRemovedByteNull);
                    byte[] ASCIIline = Encoding.ASCII.GetBytes(inputData);

                    if (ASCIIline.Contains(ProtocolASCII.VT) && ASCIIline.Contains(ProtocolASCII.FS) && ASCIIline.Contains(ProtocolASCII.CR))
                    {
                        ProcessForResult(inputData);
                        HostSend_ACK(inputData);
        ...
        public static void HostSend_ACK(string stringData)
        {
            ...
            string stringACK = (char)ProtocolASCII.VT + @"MSH|^~\&|||||" + time + "||ACK^R01|" + controlID + "|P|2.6|||AL|NE||UNICODE UTF-8|EN^English^ISO639|" + (char)ProtocolASCII.CR;
            ...
            if (isWriteFile) Folder_File_Tool.Create_Write_File(typeof(SDBIOSENSOR).Name, true, stringACK);
            ...
            try
            {
                ...
                foreach (var line in arrayInputData)
                {
                    try
                    {
                        ...
                        if (arrayLine[0] == OBX && arrayLine[2] == NM)
                        {
                            var testCodeIn = arrayLine[3].Split('^')[1];
