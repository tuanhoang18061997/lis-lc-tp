...
{
    public class MindrayBC5380_IP
    {
        private static readonly ILog log4net = LogManager.GetLogger(typeof(MindrayBC5380_IP).Name);
        ...
        private static string OBR = "OBR";
        private static string OBX = "OBX";
        ...
        public static void ServerReceive(System.Object obj)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (isWriteFile)
                        Folder_File_Tool.CreateWriteFileByDate(typeof(MindrayBC5380_IP).Name, true, stringData);
                    byteStringData = Encoding.ASCII.GetBytes(stringData);
                    if (byteStringData != null && byteStringData.Contains(ProtocolASCII.VT) && byteStringData.Contains(ProtocolASCII.FS) && byteStringData.Contains(ProtocolASCII.CR))
                    {
        ...
        private static void ProcessResult(string inputData)
        {
            ...
            try
            {
                ...
                foreach (var line in arrayInputData)
                {
                    ...
                    if (arrayLine[0] == OBR)
                    {
                        seq = !string.IsNullOrEmpty(arrayLine[3]) ? arrayLine[3] : arrayLine[2];
                    }
                    if (arrayLine[0] == OBX)
                    {
                        var testCodeIn = arrayLine[3].Split('^', StringSplitOptions.RemoveEmptyEntries).ElementAtOrDefault(1);
                        var resultTmp = arrayLine[5].Trim();
                        double? result = null;
                        var posneg = string.Empty;
                        Folder_File_Tool.ParseResult(resultTmp, out result, out posneg);
                        var map = MapBL.Get_MapByTestCodeIn(deviceID, testCodeIn);
        ...
        //    string stringACK = (char)ProtocolASCII.VT + @"MSH|^~\&|||BC-5380|Mindray|" + time + "||ACK|" + controlID + (char)ProtocolASCII.CR + (char)ProtocolASCII.LF;
        //    stringACK += (char)ProtocolASCII.FS + "SA||AA|1131||||||UNICODE" + (char)ProtocolASCII.CR + (char)ProtocolASCII.LF;
        //    byte[] byteData = Encoding.ASCII.GetBytes(stringACK);
        //    if (isWriteFile) Folder_File_Tool.Create_Write_File(typeof(MindrayBC5380_IP).Name, true, stringACK);
