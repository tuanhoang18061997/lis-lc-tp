...
{
    public class TDCN_GetSampleController : Controller
    {
        public readonly ILogger<TDCN_GetSampleController> _logger;
        ...
        public TDCN_GetSampleController(ILogger<TDCN_GetSampleController> logger, PatientCDHABL patientCDHABL, ObjectBL objectBL, LocationBL locationBL,
        ...
        public async Task<IActionResult> GetSample()
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatient(from, to, true, false, false, _TDCN);
            ...
            ViewData["lstService"] = await _serviceBL.GetListServiceByCategory(_TDCN);
        ...
        public async Task<IActionResult> Refresh()
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatientByPidOrSid(from, to, true, false, false, null, _TDCN);

            return PartialView("_TDCN_GetSample_ListPatient");
        }
        ...
        public async Task<IActionResult> Search(string pidorseq, DateTime timeSearchFrom, DateTime timeSearchTo)
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatientByPidOrSid(from, to, true, false, false, pidorseq, _TDCN);
        ...
        public async Task<IActionResult> GetPatientInfo(long id)
        {
            ...
            return PartialView("_TDCN_GetSample_PatientInfo");
        }
        ...
        public async Task<IActionResult> GetServiceForPatient(long patientId)
        {
            ...
            ViewData["lstService"] = await _serviceBL.GetListServiceByCategory(_TDCN);
            return PartialView("_TDCN_GetSample_ListService");
        }
        ...
        public async Task<IActionResult> DeletePatientAndService(long id)
        {
            ...
            {
                _status = await _patientCDHABL.DeleteById(id, _TDCN) ? "/TDCN_GetSample/Refresh/" : string.Empty;
            }
                                                               ...
                                                               string address, string diagnostic, string service)
        {
            ...
            var _patient = await _patientCDHABL.SaveOrUpdate(_id, patientId, seq, sid, patientName, age, sex, obj, type, location, doctor, getSampleTime, address, diagnostic, _userLogin.Value, _dateTime, true, false, false, _TDCN);
        ...
        public async Task<IActionResult> AddServiceForPatient(long patientId, long serviceId, long doctorId)
        {
            var _userLogin = this.GetUserLogin();
            var _dateTime = ToolBL.Get_DateNow();
            var _save = await _resultCDHABL.SaveService(patientId, serviceId, _userLogin.Value, _dateTime, doctorId, _TDCN.ToLower());
        ...
        {
            var _exitResultCDHA = await _resultCDHABL.ExitResultXN_Update_GetSampleTime(id, getSampleTime, _TDCN);
            var _getSample = false;
            if (_exitResultCDHA)
            {
                var _userLogin = this.GetUserLogin();
                _getSample = await _patientCDHABL.GetSample_ProcessResult_ReturnResult(id, false, true, false, _userLogin.Value, _TDCN);
            }
        ...
        public async Task<IActionResult> ProcessResultAll([FromBody] List<long> patientIds)
        {
            ...
            foreach (var patientId in patientIds)
            {
                try
                {
                    ...
                    var _exitResultCDHA = await _resultCDHABL.ExitResultXN_Update_GetSampleTime(patientId, ToolBL.Get_DateNow(), _TDCN);
                    if (_exitResultCDHA)
                    {
                        var _getSample = await _patientCDHABL.GetSample_ProcessResult_ReturnResult(patientId, false, true, false, _userLogin.Value, _TDCN);
                        if (_getSample)
                        {
                            successCount++;
                        }
