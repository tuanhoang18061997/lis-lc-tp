...
{
    public class NS_GetSampleController : Controller
    {
        public readonly ILogger<NS_GetSampleController> _logger;
        ...
        public NS_GetSampleController(ILogger<NS_GetSampleController> logger, PatientCDHABL patientCDHABL, ObjectBL objectBL, LocationBL locationBL,
                            DoctorBL doctorBL, UserBL userBL, CategoryBL categoryBL, ServiceBL serviceBL, ResultCDHABL resultCDHABL, SettingBL settingBL, GroupBL groupBL)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> GetSample()
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatient(from, to, true, false, false, _NS);
            ...
            ViewData["lstService"] = await _serviceBL.GetListServiceByCategory(_NS);
        ...
        public async Task<IActionResult> Refresh()
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatientByPidOrSid(from, to, true, false, false, null, _NS);

            return PartialView("_NS_GetSample_ListPatient");
        }
        ...
        public async Task<IActionResult> Search(string pidorseq, DateTime timeSearchFrom, DateTime timeSearchTo)
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatientByPidOrSid(from, to, true, false, false, pidorseq, _NS);
        ...
        public async Task<IActionResult> GetPatientInfo(long id)
        {
            ...
            return PartialView("_NS_GetSample_PatientInfo");
        }
        ...
        public async Task<IActionResult> GetServiceForPatient(long patientId)
        {
            ...
            ViewData["lstService"] = await _serviceBL.GetListServiceByCategory(_NS);
            return PartialView("_NS_GetSample_ListService");
        }
        ...
        public async Task<IActionResult> DeletePatientAndService(long id)
        {
            ...
            {
                _status = await _patientCDHABL.DeleteById(id, _NS) ? "/NS_GetSample/Refresh/" : string.Empty;
            }
                                                               ...
                                                               string address, string diagnostic, string service)
        {
            ...
            var _patient = await _patientCDHABL.SaveOrUpdate(_id, patientId, seq, sid, patientName, age, sex, obj, type, location, doctor, getSampleTime, address, diagnostic, _userLogin.Value, _dateTime, true, false, false, _NS);
        ...
        public async Task<IActionResult> AddServiceForPatient(long patientId, long serviceId, long doctorId)
        {
            var _userLogin = this.GetUserLogin();
            var _dateTime = ToolBL.Get_DateNow();
            var _save = await _resultCDHABL.SaveService(patientId, serviceId, _userLogin.Value, _dateTime, doctorId, _NS.ToLower());
        ...
        {
            var _exitResultCDHA = await _resultCDHABL.ExitResultXN_Update_GetSampleTime(id, getSampleTime, _NS);
            var _getSample = false;
            if (_exitResultCDHA)
            {
                var _userLogin = this.GetUserLogin();
                _getSample = await _patientCDHABL.GetSample_ProcessResult_ReturnResult(id, false, true, false, _userLogin.Value, _NS);
            }          
