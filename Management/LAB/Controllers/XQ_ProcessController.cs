...
{
    public class XQ_ProcessController : Controller
    {
        private readonly ILogger<XQ_ProcessController> _logger;
        ...
        public XQ_ProcessController(ILogger<XQ_ProcessController> logger, PatientCDHABL patientCDHABL, ObjectBL objectBL, LocationBL locationBL,
        ...
        public async Task<IActionResult> Process()
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatient_Flexible(from, to, _XQ, "process");
            ViewData["lstUser"] = await _userBL.GetListUser(UserTypeModel.XQ);
            ViewData["lstBenhAnModel"] = await BenhAnModel.GetListBenhAnModel();
            ViewData["lstSampleResult"] = await _sampleBL.GetListSampleByCategory(_XQ);
        ...
        public async Task<IActionResult> Get_Count()
        {
            ...
            var countGetSample = await _patientCDHABL.Get_CountPatient_New(from, to, true, false, false, _XQ);
            var countProcess = await _patientCDHABL.Get_CountPatient_New(from, to, false, true, false, _XQ);
            var countReturnResult = await _patientCDHABL.Get_CountPatient_New(from, to, false, false, true, _XQ);
        ...
        public async Task<IActionResult> Refresh()
        {
            ...
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatientByPidOrSid_New(from, to, false, true, false, null, _XQ);

            return PartialView("_XQ_Process_ListPatient");
        }
        ...
        public async Task<IActionResult> Search(string pidorseq, DateTime timeSearchFrom, DateTime timeSearchTo)
        {
            ...
            var from = new DateTime(timeSearchFrom.Year, timeSearchFrom.Month, timeSearchFrom.Day, 00, 00, 00).AddDays(-7);
            var to = new DateTime(timeSearchTo.Year, timeSearchTo.Month, timeSearchTo.Day, 23, 59, 59);
            ViewData["lstPatient"] = await _patientCDHABL.Get_ListPatientByPidOrSid_New(from, to, false, true, false, pidorseq, _XQ);
        ...
        public async Task<IActionResult> GetSample(long id)
        {
            ...
            var _getSample = await _patientCDHABL.GetSample_ProcessResult_ReturnResult(id, true, false, false, _userLogin.Value, _XQ);
        ...
        public async Task<IActionResult> GetPatientInfo(long id)
        {
            ...
            return PartialView("_XQ_Process_PatientInfo");
        }
        ...
        public async Task<IActionResult> GetServiceForPatient(long patientId)
        {
            ...
            return PartialView("_XQ_Process_ListService");
        }
        ...
        public async Task<IActionResult> SaveResult([FromBody] ResultCDHAModel resultCDHA)
        {
            ...
            if (resultCDHA != null)
            {
                ...
                await _patientCDHABL.Update(resultCDHA.patientId, resultCDHA.returnResultTime, resultCDHA.userReturnResult, _userLogin.Value, _dateTime, _XQ);
                _saveResult = await _resultCDHABL.Update(resultCDHA, resultCDHA.userReturnResult, _dateTime, _device);
            }
        ...
        public async Task<IActionResult> ValidPrint([FromBody] ResultCDHAModel resultCDHA)
        {
            if (resultCDHA != null)
            {
                ...
                {
                    if (await _resultCDHABL.ValidateCDHAAsync(resultCDHA, _XQ, _userLogin.Value, _dateTime))
                    {
                        ...
                        if (_result != null)
                        {
                            try
                            {
                                ...
                                var _folder = Path.Combine(_environment.WebRootPath, "pdf", "xq");
        ...
        public async Task<IActionResult> SelectDevice(long deviceId)
        {
            ...
            try
            {
                ...
                if (_device != null)
                {
                    ...
                    HttpContext.Response.Cookies.Append(SessionKeyModel._sessionDeviceXQ, value, cookieOptions);
                    _status = true;
                }
            }
            catch { }
        ...
        {
            string _value = HttpContext.Request.Cookies[SessionKeyModel._sessionDeviceXQ];
            Device _device = null;
        ...
        public async Task<IActionResult> UploadHinh(IFormFile file, string resultCDHAId)
        {
            ...
            var _folder = Path.Combine(_environment.WebRootPath, "images", "result", "xq");
            ...
            if (_saveImage)
            {
                var _imagePath = "/images/result/xq/" + resultCDHAId + "-" + guid + ".png";
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> ScanFolderAndSyncImages(long resultCDHAId, string patientCode)
        {
            // 1) Thư mục CVS-03 export (đổi theo thực tế nếu khác)
            var xqFolder = Path.Combine(_environment.WebRootPath, "xq_export");
            if (!Directory.Exists(xqFolder)) return Content("False");

            // 2) Thư mục lưu dấu file đã import để tránh trùng
            var importedRoot = Path.Combine(xqFolder, "_imported");
            ...
            // 4) Lấy các file ảnh ở ngay thư mục xq_export (không đệ quy)
            var candidates = Directory.EnumerateFiles(xqFolder, "*", SearchOption.TopDirectoryOnly)
                                      .Where(p => allowedExts.Contains(Path.GetExtension(p)))
                                      .OrderByDescending(p => new FileInfo(p).LastWriteTimeUtc)
                                      .ToList();

            if (candidates.Count == 0) return Content("False");

            // 5) Nơi lưu trữ cuối (web hiển thị)
            var destFolder = Path.Combine(_environment.WebRootPath, "images", "result", "xq");
            ...
            foreach (var src in candidates)
            {
                try
                {
                    // Đợi file upload/ghi xong (giữ nguyên util IsFileReady của bạn)
                    if (!IsFileReady(src))
                    {
                        Thread.Sleep(300);
                        if (!IsFileReady(src)) continue;
                    }
                    ...
                    // Lưu DB: web path + physical path
                    var webPath = "/images/result/xq/" + destName;
                    var ok = await _resultCDHABL.SaveImage(resultCDHAId, webPath, destPath);
                    if (ok)
                    {
                        importedCount++;

                        // 6) Move nguồn sang _imported/yyyyMMdd để đánh dấu đã dùng
                    ...
                    {
                        // Nếu lưu DB thất bại, có thể xoá file đích để tránh rác (tuỳ nhu cầu)
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> UploadXQAndSync(long resultCDHAId, string patientCode, IFormFile file)
        {
            try
            {
                ...
                var ecgExport = Path.Combine(_environment.WebRootPath, "xq_export");
