...
function Process_GetUploadPdfSelectedResultId() {
    ...
    var $checked = $('#tdcn_upload_pdf_list_service .tdcn-upload-pdf-chk-service:checked').first();
    if ($checked.length && $checked.val()) return $checked.val();

    // Fallback nếu sau này UI vẫn dùng class row-service cũ.
    var fallbackId = '';
    $('.row-service .form-check-input:checked').each(function () {
        if (!fallbackId) fallbackId = $(this).val();
    });
    return fallbackId;
}
...
function Process_LoadImageForService(id) {
    ...
    $containHinh.empty();
    CKEDITOR.instances["tdcn_process_description"].setData("");
    $("#tdcn_process_result").val("");
    $("#tdcn_process_suggest").val("");
    $.ajax({
        ...
        success: function (response) {
            $.each(response, function (key, value) {
                ...
                CKEDITOR.instances["tdcn_process_description"].setData(value.description);
                $("#tdcn_process_result").val(value.result);
                $("#tdcn_process_suggest").val(value.suggest);
            });
        }
    });
}
